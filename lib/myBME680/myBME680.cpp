...
void BME680::getCalibration()
{
  ...
  TWI_readBytes(BME680_SLAVE_ADDRESS, BME680_COEFF_START_ADDRESS1, BME680_COEFF_SIZE1, &coeff_array1[0]); // Split reading registers into 2
  TWI_readBytes(BME680_SLAVE_ADDRESS, BME680_COEFF_START_ADDRESS2, BME680_COEFF_SIZE2, &coeff_array2[0]); // one 25 bytes and the other 16
    /*************************************
...
const uint32_t lookupTable2[16]  = {
  ...
  TWI_readBytes(BME680_SLAVE_ADDRESS, BME680_STATUS_REGISTER, 15, &buff[0]);                   // read all 15 bytes in one go
